...
class Http {
  ...
  constructor() {
    ...
    this.instance.interceptors.request.use(
      (config) => {
        ...
        if ((config.url === URL.LOGOUT || config.url?.endsWith(URL.LOGOUT)) && session?.refreshToken) {
          config.data = {
            ...(config.data ?? {}),
  ...
  private async handleRefreshToken(): Promise<string> {
    ...
    this.refreshTokenRequest = this.instance
      ...
      .then(async (res) => {
        const newAccessToken = (res.data as ApiResponse<{ accessToken: string }>).data.accessToken
